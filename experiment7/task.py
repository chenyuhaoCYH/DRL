...
class Task:
    ...
    def __init__(self, vehicle=None, createTime=0, flag=1):
        ...
        if flag == 1:
            # 娱乐性任务
            self.max_time = 60  # np.random.randint(50, 70)  # ms  最大容忍时间
        else:
            # 安全性任务
            self.max_time = 50  # np.random.randint(40, 50)
        self.size = np.random.uniform(0.3, 0.5)  # np.random.uniform(0.5, 1)  # Mb
