...
gama = 1.25 * (10 ** -26)  # 能量系数 J/M cycle
a = 0.6  # 奖励中时间占比
b = 0.4  # 奖励中能量占比
T1 = -0.5
T2 = -0.05
...
class Env:
    ...
    def compute_hold_time(self):
        ...
        #             while j < len(vehicle.task_queue_for_reward):
        #                 vehicle.task_queue_for_reward[j].hold_time += cur_need_time[0]
        #                 j += 1
        #             cur_need_time.pop(0)
        #             # 每个任务减去最小时间
        ...
        #             cur_need_time.sort()
        # # 对mec
        # if len(self.MEC.task_queue) > 0:
        #     self.MEC.task_queue_for_reward = [task for task in self.MEC.task_queue_for_reward]
        #     for task in self.MEC.task_queue_for_reward:
        #         task.hold_time = 0
        #     # 记录当前正在处理任务得所需时间(且已经从小到大排序)
        #     cur_need_time = []
        ...
        #         while j < len(self.MEC.task_queue_for_reward):
        #             self.MEC.task_queue_for_reward[j].hold_time += cur_need_time[0]
        #             j += 1
        #         cur_need_time.pop(0)
        #         # 每个任务减去最小时间
        #         cur_need_time = [item - task.hold_time for item in cur_need_time]
    ...
    def get_reward(self, task, avg_time):
        ...
        sum_time = trans_time + compute_time + np.abs(task.pick_time - task.create_time + 50) % 50 + task.hold_time
        ...
        if task.aim != vehicle:
            ...
            # 计算任务消耗的能量（本地卸载）
            price = 0
            energy = round(gama * np.power(task.compute_resource * (10 ** 6), 3) * compute_time / 10, 2)
